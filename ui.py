...
class gui(ui):
    ...
    ############################################################
    SMALLFONT = '{Copperplate Gothic Light} 14'
    LARGEFONT = '{Copperplate Gothic Bold} 30'
    TINYFONT = '{Copperplate Gothic Light} 12'
    ...
    def __init__(self, network):
        ...
        Button(frame, text="Log In", font=gui.SMALLFONT, command=self.__logIn).pack(fill=X)
        Label(frame, text="", bg='#9DE3FD').pack(fill=X)
        Button(frame, text="Create Account", font=gui.SMALLFONT, command=self.__createAccount).pack(fill=X)
        Button(frame, text="Play As Guest", font=gui.SMALLFONT, command=self.__guestLogIn).pack(fill=X)
        Label(frame, text="", bg='#9DE3FD').pack(fill=X)
        Button(self.__quitFrame, text="Quit", font=gui.SMALLFONT, command=self.__quit).pack(fill=X)
        Label(frame, text="", bg='#9DE3FD').pack(fill=X)
    ...
    def __createAccount(self):
        if self.__network:
            ...
            if not self.__createAccountFrame:
                ...
                # Set up the window
                Label(frame, text="Create Account", bg='#9DE3FD', font=gui.LARGEFONT, pady=25, padx=45).pack(fill=X)
                ...
                Button(frame, text="Create Account", command=self.__addAccount, font=gui.SMALLFONT).pack(fill=X)
                Label(frame, text="", bg='#9DE3FD').pack(fill=X)
                console = Listbox(frame, height=3, width=100)
                console.pack()
                self.__createAccountConsole = console
                Button(frame, text="Cancel", command=self.__cancelCreate, font=gui.SMALLFONT).pack(fill=X)
    ...
    def __menu(self):
        ...
        if not self.__menuFrameCreated:
            ...
            frame.pack()
            # Set up the window
            Label(frame, text="Menu", bg='#9DE3FD', font=gui.LARGEFONT, pady=25, padx=45).pack(fill=X)
            Button(frame, text="Play", command=self.__gametype, font=gui.SMALLFONT).pack(fill=X)
            Button(frame, text="Help", command=self.__help, font=gui.SMALLFONT).pack(fill=X)
            Button(frame, text="Stats", command=self.__stats, font=gui.SMALLFONT).pack(fill=X)
            Label(frame, text="", bg='#9DE3FD').pack(fill=X)
            Button(frame, text="Log Out", command=self.__dismissMenu, font=gui.SMALLFONT).pack(fill=X)
    ...
    def __stats(self):
        ...
        if not self.__guest:
            ...
            # Set up the window
            Label(frame, text=f"{self.__username.get()}'s stats:", bg='#9DE3FD', font=gui.LARGEFONT).pack(fill=X)
            Label(frame, text=f"PLAYED: {self.__statsPlayed}", bg='#9DE3FD', font=gui.TINYFONT).pack(fill=X)
            Label(frame, text=f"WON: {self.__statsWon}", bg='#9DE3FD', font=gui.TINYFONT).pack(fill=X)
            Label(frame, text=f"LOST: {self.__statsLost}", bg='#9DE3FD', font=gui.TINYFONT).pack(fill=X)
            Label(frame, text=f"DRAWN: {self.__statsDrawn}", bg='#9DE3FD', font=gui.TINYFONT).pack(fill=X)
            Label(frame, text=f"PUZZLE WINS: {self.__statsPFin}", bg='#9DE3FD', font=gui.TINYFONT).pack(fill=X)
            Label(frame, text=f"PUZZLES MADE: {self.__statsPMade}", bg='#9DE3FD', font=gui.TINYFONT).pack(fill=X)
            Button(frame, text="Dismiss", command=self.__dismissStats, font=gui.SMALLFONT).pack(fill=X)
    ...
    def __gametype(self):
        ...
        if not self.__typeChoiceCreated:
            ...
            # Set up the window
            Label(frame, text="Play", bg='#9DE3FD', font=gui.LARGEFONT, pady=25, padx=45).pack(fill=X)
            Button(frame, text="Pass and Play", command=self.__gameSetup, font=gui.SMALLFONT).pack(fill=X)
            if self.__network:
                Button(frame, text="Play Local Online", command=self.__LANSetup, font=gui.SMALLFONT).pack(fill=X)
                Button(frame, text="Puzzles", command=self.__puzzleSetup, font=gui.SMALLFONT).pack(fill=X)
            Label(frame, text="", bg='#9DE3FD').pack(fill=X)
            Button(frame, text="Dismiss", command=self.__dismissTypeChoice, font=gui.SMALLFONT).pack(fill=X)
    ...
    def __puzzleSetup(self):
        ...
        if not self.__puzzleSetupCreated:
            ...
            # Set up the window
            Label(frameUpper, text="Puzzles", bg='#9DE3FD', font=gui.LARGEFONT, pady=25, padx=45).pack(fill=X)
            Button(frameUpper, text="Create", command=self.__puzzleCreate, font=gui.SMALLFONT).pack(fill=X)
            Button(frameUpper, text="Random", command=self.__puzzleRandom, font=gui.SMALLFONT).pack(fill=X)
            Label(frameUpper, text="", bg='#9DE3FD').pack(fill=X)
            Entry(frameUpper, textvariable=self.__puzzleCode).pack(fill=X)
            Button(frameUpper, text="Load", command=self.__puzzleLoad, font=gui.SMALLFONT).pack(fill=X)
            Label(frameUpper, text="", bg='#9DE3FD').pack(fill=X)
            ...
            Label(frameMiddle, text=f"Player 1 colour:", bg='#9DE3FD', font=gui.SMALLFONT).grid(row=0, column=0, sticky=W)
            pOneDropDown = OptionMenu(frameMiddle, self.__pOne, *self.__counters.keys())
            pOneDropDown.configure(font=gui.SMALLFONT)
            pOneDropDown.grid(row=0, column=1, sticky=W)

            Label(frameMiddle, text=f"Player 2 colour:", bg='#9DE3FD', font=gui.SMALLFONT).grid(row=1, column=0, sticky=W)
            pTwoDropDown = OptionMenu(frameMiddle, self.__pTwo, *self.__counters.keys())
            pTwoDropDown.configure(font=gui.SMALLFONT)
            pTwoDropDown.grid(row=1, column=1, sticky=W)
            Label(frameMiddle, text=f"Shape:", bg='#9DE3FD', font=gui.SMALLFONT).grid(row=2, column=0, sticky=W)

            self.__shape = StringVar()
            self.__shape.set("CIRCLE")
            shapes = ["CIRCLE", "SQUARE", "TRIANGLE"]
            shapeDropDown = OptionMenu(frameMiddle, self.__shape, *shapes)
            shapeDropDown.configure(font=gui.SMALLFONT)
            shapeDropDown.grid(row=2, column=1, sticky=W)

            Label(frameMiddle, text="Board Size:", bg='#9DE3FD', font=gui.SMALLFONT).grid(row=3, column=0, sticky=W)
            ...
            Button(frameLower, text="Dismiss", font=gui.SMALLFONT, command=self.__dismissPuzzleSetup).pack(fill=X)
            Label(frameLower, text="", bg='#9DE3FD').pack(fill=X)
    ...
    def __puzzleGame(self, create=False):
        ...
        for col in range(7):
            ...
            Button(frameButtons, textvariable=t, command=cmd, font=gui.SMALLFONT).grid(row=0, column=col, sticky=N+S+W+E)
        for col in range(7):
        ...
        Label(frameMenu, text=t, font=gui.SMALLFONT).grid(row=0, column=0, sticky=N+S+E+W)
        if create:
            self.__creative = True
            Entry(frameMenu, textvariable=self.__puzzleID).grid(row=0, column=1, sticky=N+S+W+E)
            Button(frameMenu, text="Save", command=self.__savePuzzle, font=gui.SMALLFONT).grid(row=1, column=1, sticky=N+S+W+E)
        else:
            self.__creative = False
            Label(frameMenu, text=f"ID: {self.__loadPuzzleID}", font=gui.SMALLFONT).grid(row=0, column=1, sticky=N+S+W+E)
            Button(frameMenu, text="Solve", command=self.__solvePuzzle, font=gui.SMALLFONT).grid(row=1, column=1, sticky=N+S+W+E)
        Button(frameMenu, text="Undo", command=self.__undoMove, font=gui.SMALLFONT).grid(row=1, column=0, sticky=N+S+W+E)

        frameBottom = Frame(self.__root, bg='#9DE3FD')
        frameBottom.pack()
        Button(frameBottom, text="Exit", command=self.__exitPuzzle, font=gui.SMALLFONT).pack()
    ...
    def __puzzleWin(self):
        ...
        # Set up the window
        Label(frame, text=f"CORRECT MOVE!", bg='#9DE3FD', font=gui.LARGEFONT, pady=25, padx=45).pack(fill=X)
        Label(frame, text="WELL DONE!", bg='#9DE3FD', font=gui.SMALLFONT, pady=5, padx=10).pack(fill=X)
        Button(frame, text="Exit", command=self.__exitSolved, font=gui.SMALLFONT).pack(fill=X)
    ...
    def __puzzleLose(self):
        ...
        # Set up the window
        Label(frame, text=f"INCORRECT MOVE!", bg='#9DE3FD', font=gui.LARGEFONT, pady=25, padx=45).pack(fill=X)
        Label(frame, text="UNDO OR EXIT", bg='#9DE3FD', font=gui.SMALLFONT, pady=5, padx=10).pack(fill=X)
        Button(frame, text="Exit", command=self.__exitLost, font=gui.SMALLFONT).pack(fill=X)
    ...
    def __LANSetup(self):
        ...
        if not self.__LANSetupCreated:
            ...
            # Set up the window
            Label(frameUpper, text=f"Local Play", bg='#9DE3FD', font=gui.LARGEFONT, pady=25, padx=45).pack(fill=X)
            Button(frameUpper, text="Host", command=self.__hostGame, font=gui.SMALLFONT).pack(fill=X)
            Label(frameUpper, text="", bg='#9DE3FD').pack(fill=X)
            Entry(frameUpper, textvariable=self.__joinCode).pack(fill=X)
            Button(frameUpper, text="Join", command=self.__attemptJoin, font=gui.SMALLFONT).pack(fill=X)
            Label(frameUpper, text="", bg='#9DE3FD').pack(fill=X)
            ...
            frameLower.pack()
            Label(frameLower, text="", bg='#9DE3FD').pack(fill=X)
            Button(frameLower, text="Back", command=self.__dismissLANSetup, font=gui.SMALLFONT).pack(fill=X)
            Label(frameLower, text="", bg='#9DE3FD').pack(fill=X)
            console = Listbox(frameLower, height=3, width=100)
            console.pack()
    ...
    def __hostGame(self):
        ...
        # Set up the window
        Label(frame, text=f"Host Game", bg='#9DE3FD', font=gui.LARGEFONT, pady=25, padx=45).pack(fill=X)
        Label(frame, text=f"JOIN CODE: {self.__clientCode}", bg='#9DE3FD', font=gui.SMALLFONT, pady=5, padx=10).pack(fill=X)
        Label(frame, text="WAITING FOR OPPONENT...", bg='#9DE3FD', font=gui.SMALLFONT, pady=5, padx=10).pack(fill=X)
        Label(frame, text="", bg='#9DE3FD').pack(fill=X)
        Button(frame, text="Back", command=self.__dismissHost, font=gui.SMALLFONT).pack(fill=X)
    ...
    def __gameSetup(self):
        ...
        # Set up the window
        Label(frameUpper, text=f"Play", bg='#9DE3FD', font=gui.LARGEFONT, pady=25, padx=45).pack(fill=X)
        self.__firstTurn = StringVar()
        ...
        if not self.__guest:
            ...
            firstTurnDropDown.configure(font=gui.SMALLFONT)
            firstTurnDropDown.pack(fill=X)
        frameMiddle = Frame(self.__root, bg='#9DE3FD')
        frameMiddle.pack()

        Label(frameMiddle, text="Choose Opponent:", bg='#9DE3FD', font=gui.SMALLFONT).grid(row=0, column=0, sticky=W)
        ...
        opponentDropDown.configure(font=gui.SMALLFONT)
        opponentDropDown.grid(row=0, column=1, sticky=W)

        playerOne = "Player 1" if self.__guest else f"{self.__username.get()}"
        Label(frameMiddle, text=f"{playerOne} colour:", bg='#9DE3FD', font=gui.SMALLFONT).grid(row=1, column=0, sticky=W)
        playerTwo = "Player 2" if self.__guest else f"Opponent"
        Label(frameMiddle, text=f"{playerTwo} colour:", bg='#9DE3FD', font=gui.SMALLFONT).grid(row=2, column=0, sticky=W)
        self.__pOne = StringVar()
        self.__pOne.set("RED")
        self.__pTwo = StringVar()
        self.__pTwo.set("YELLOW")
        ...
        Label(frameMiddle, text="Shape:", bg='#9DE3FD', font=gui.SMALLFONT).grid(row=3, column=0, sticky=W)
        self.__shape = StringVar()
        self.__shape.set("CIRCLE")
        shapes = ["CIRCLE", "SQUARE", "TRIANGLE"]
        shapeDropDown = OptionMenu(frameMiddle, self.__shape, *shapes)
        ...
        Label(frameMiddle, text="Board Size:", bg='#9DE3FD', font=gui.SMALLFONT).grid(row=4, column=0, sticky=W)
        self.__boardSize = Scale(frameMiddle, from_=40, to=135, orient=HORIZONTAL)
        self.__boardSize.set(120)
        ...
        frameLower.pack()
        Label(frameLower, text="", bg='#9DE3FD').pack(fill=X)
        Button(frameLower, text="Play", command=self.__play, font=gui.SMALLFONT).pack(fill=X)
        Button(frameLower, text="Load", command=self.__load, font=gui.SMALLFONT).pack(fill=X)
        Label(frameLower, text="", bg='#9DE3FD').pack(fill=X)
        Button(frameLower, text="Back", command=self.__dismissSetup, font=gui.SMALLFONT).pack(fill=X)
    ...
    def __load(self):
        ...
        if not self.__loadCreated:
            ...
            # Set up the window
            Label(frame, text=f"Load", bg='#9DE3FD', font=gui.LARGEFONT, pady=25, padx=45).pack(fill=X)
            self.__loadName = StringVar()
            self.__loadName.set("Enter Game Name")
            Label(frame, text=f"ENTER GAME NAME BELOW", bg='#9DE3FD', font=gui.SMALLFONT).pack(fill=X)
            Entry(frame, textvariable=self.__loadName).pack(fill=X)
            Button(frame, text="Load", command=self.__loadGame, font=gui.SMALLFONT).pack(fill=X)
            Label(frame, text="", bg='#9DE3FD').pack(fill=X)
            Button(frame, text="Cancel", command=self.__cancelLoad, font=gui.SMALLFONT).pack(fill=X)
            Label(frame, text="", bg='#9DE3FD').pack(fill=X)
            console = Listbox(frame, height=3, width=100)
            console.pack()
    ...
    def __help(self):
        ...
        if not self.__helpCreated:
            ...
            # Set up the window
            Label(frame, text=f"Help", bg='#9DE3FD', font=gui.LARGEFONT, pady=25, padx=45).pack(fill=X)
            Label(frame, text="Take it in turns to  drop counters into the board.", bg='#9DE3FD', font=gui.SMALLFONT).pack(fill=X)
            Label(frame, text="First person to get a vertical, horizontal or diagonal run of four counters wins!", bg='#9DE3FD', font=gui.SMALLFONT).pack(fill=X)
            Label(frame, text="If the board fills and nobody has a run of four yet then the game is drawn.", bg='#9DE3FD', font=gui.SMALLFONT).pack(fill=X)
            Label(frame, text="", bg='#9DE3FD').pack(fill=X)
            Button(frame, text="Dismiss", command=self.__dismissHelp, font=gui.SMALLFONT).pack(fill=X)
    ...
    def __play(self):
        ...
        for col in range(7):
            ...
            Button(frameButtons, textvariable=t, command=cmd, font=gui.SMALLFONT).grid(row=0, column=col, sticky=E + W)
        for col in range(7):
            Grid.columnconfigure(frameButtons, col, weight=1)
        ...
        if self.__opponentType.get() == "Human" and not self.__localGame:
            ...
            self.__playerTurn.set('YOUR TURN\nCHOOSE COLUMN')
        self.__playerTurnLabel = Label(frameMenu, textvariable=self.__playerTurn, fg=textColour, font=gui.SMALLFONT)
        self.__playerTurnLabel.grid(row=0, column=0, sticky=N+S+E+W)
        Button(frameMenu, text="Dismiss", command=self.__dismissGame, font=gui.SMALLFONT).grid(row=1, column=0, sticky=N+S+E+W)

        if not self.__localGame:
            Button(frameMenu, text="Undo", command=self.__undoMove, font=gui.SMALLFONT).grid(row=0, column=1, sticky=N+S+E+W)
            Button(frameMenu, text="Save and Exit", command=self.__saveAndExit, font=gui.SMALLFONT).grid(row=1, column=1, sticky=N+S+E+W)
        ...
        self.__gameConsole = console
        if self.__localGame:
            self.__gameConsole.insert(END, f"1| network game (vs {self.__opponent})")
        else:
    ...
    def __saveAndExit(self):
        if self.__animating:
            ...
            if not self.__saveCreated:
                ...
                # Set up the window
                Label(frame, text=f"Save", bg='#9DE3FD', font=gui.LARGEFONT, pady=25, padx=45).pack(fill=X)
                Label(frame, text=f"ENTER GAME NAME BELOW", bg='#9DE3FD', font=gui.SMALLFONT).pack(fill=X)
                Entry(frame, textvariable=self.__gameName).pack(fill=X)
                Button(frame, text="Save", command=self.__saveGame, font=gui.SMALLFONT).pack(fill=X)
                Label(frame, text="", bg='#9DE3FD').pack(fill=X)
                Button(frame, text="Cancel", command=self.__cancelSave, font=gui.SMALLFONT).pack(fill=X)
                Label(frame, text="", bg='#9DE3FD').pack(fill=X)
                console = Listbox(frame, height=3, width=100)
